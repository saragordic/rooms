...
- **A room on each display.** With a second screen, each display can show its own room: press ⌥Space on the monitor and pick one room, then on the laptop pick another. Walking into a room only replaces the room on the display you're on; the other one stays as it is. Choosing a room that's out on the other display (⌥Space marks it) brings it over.
